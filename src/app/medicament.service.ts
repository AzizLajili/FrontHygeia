...
export class MediacamentService  {
  constructor(private http:HttpClient ) { }

 

  getMedicamentsList():Observable<any[]>{
    return this.http.get<any[]> ('http://localhost:8090/SpringMVC/medicaments/sortedByPrice');
  }
  getMedicamentByName(nom: string): Observable<Medicament> {
    return this.http.get<Medicament>(`http://localhost:8090/SpringMVC/medicaments/getByMedicamentName/${nom}`);
  }
